...
{
    if(CLK->PWRCTL & CLK_PWRCTL_HXT_EN_Msk )
        return __HXT;
    else
        return 0;
}
...
{
    if(CLK->PWRCTL & CLK_PWRCTL_LXT_EN_Msk )
        return __LXT;
    else
        return 0;
}
...
uint32_t CLK_GetPLLClockFreq(void)
{
    ...
    u32NO = (u32PllReg & CLK_PLLCTL_OUT_DV_Msk) ? 2: 1;
...
/**
  * @brief  This function set HCLK frequency. The frequency unit is Hz. The range of u32Hclk is 24 ~ 32 MHz
...
uint32_t CLK_SetCoreClock(uint32_t u32Hclk)
{
    ...
    if(u32Hclk>FREQ_32MHZ) u32Hclk=FREQ_32MHZ;
    ...
    /* Select HCLK clock source to PLL,
       Select HCLK clock source divider as 3
