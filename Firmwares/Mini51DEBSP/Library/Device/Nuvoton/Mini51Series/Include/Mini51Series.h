...
/**
   ...
   */
/**
  * \page pg1 NuMicro Mini51DE BSP Directory Structure
  ...
  * \li Removed PLL related API declarations in clk.h.
  * \li Removed DID related functions and macros in fmc.c and fmc.h.
  * \li Removed FMC_SetBootSource(), FMC_DisableAPUpdate(), FMC_DisableConfigUpdate(), FMC_DisableLDUpdate(), FMC_EnableAPUpdate(),
  *             FMC_EnableConfigUpdate(), FMC_EnableLDUpdate() functions in fmc.c and fmc.h because there exist functionally identical macros.
  * \li Removed SPI_GET_RX_FIFO_FULL_FLAG() macro in spi.h.
  * \li Removed CHIP_RST and CPU_RST definitions in sys.h.
  * \li Updated startup_Mini51Series.s and retarget.c to implement a Hard Fault handler that dumps register value before going to an infinite loop.
  * \li Updated return type of CLK_WaitClockReady() function from void to uint32_t in clk.c.
  * \li Updated SPI_Close() to remove the code that disable SPI peripheral clock in spi.c.
  * \li Updated all IAR samples to set Reset_Handler() as application entry point.
  * \li Updated fmc_ld_iap sample to reduce code size by replacing retarget.c with ldrom_retarget.c.
  * \li Updated I2C_Interrupt_EEPROM sample to poll STOP bit after STOP command.
  * \li Updated DeepSleep sample so that CPU is in unlock state before entering Power-down mode.
  * \li Fixed CLK_SysTickDelay() bug that delay time is sometimes incorrect in clk.c.
  * \li Fixed FMC_Erase() bug that modifies irrelevant register bits in fmc.c.
  * \li Fixed GPIO_DISABLE_DOUT_MASK() and GPIO_ENABLE_DOUT_MASK() error that sets and clear wrong bits in gpio.h.
  * \li Fixed I2C_GetBusClockFreq() bug that reports wrong clock frequency in i2c.c.
  * \li Fixed PWM_FB1_ACMP0 definition error in pwm.h.
  * \li Fixed PWM_ENABLE_OUTPUT_INVERTER() bug that set wrong bit in pwm.h
  * \li Fixed SPI_EnableAutoSS() register setting error in spi.c.
  * \li Fixed SPI_SetBusClock() clock calculation error in spi.c.
  * \li Fixed SYS_DISABLE_BOD() and SYS_CLEAR_RST_SOURCE() implementation error in sys.h
  * \li Fixed UART_EnableFlowCtrl() register setting error in uart.c.
  * \li Fixed UART_Open() and UART_SetLine_Config() baud rate calculation error in uart.c while UART peripheral clock divider is not 1.
  * \li Fixed UART_SelectIrDAMode() error that only supports 12MHZ crystal as clock source in uart.c.
  ...
  * \li Fixed CMSIS path setting error in some sample codes.
  * \li Fixed UART_RS485 samples that set RTS pin and RS485 mode incorrectly.
  * \li Added I2C0, SPI0, and UART0 definition which is the same with I2C, SPI, and UART in Mini51Series.h.
  * \li Added ACMP driver acmp.c and acmp.h.
  * \li Added CLK_EnableSysTick() and CLK_DisableSysTick() in clk.c.
  * \li Added CLK_PWRCON_XTL12M, CLK_PWRCON_HXT, CLK_PWRCON_LXT, CLK_CLKSEL0_HCLK_S_LIRC, CLK_CLKSEL0_HCLK_S_HIRC, CLK_CLKSEL0_STCLK_S_HIRC_DIV2,
  *           CLK_CLKSEL1_WDT_S_LIRC, CLK_CLKSEL1_ADC_S_HIRC, CLK_CLKSEL1_TMR0_S_LIRC, CLK_CLKSEL1_TMR0_S_HIRC, CLK_CLKSEL1_TMR1_S_LIRC,
  *           CLK_CLKSEL1_TMR1_S_HIRC, CLK_CLKSEL1_UART_S_HIRC, CLK_CLKSEL2_FRQDIV_HXT, CLK_CLKSEL2_FRQDIV_LXT, CLK_CLKSEL2_FRQDIV_HIRC definitions
  *           in clk.h.
  * \li Added FMC_CLR_FAIL_FLAG() macro in fmc.h.
  * \li Added FMC_GetVectorPageAddr() function in fmc.c.
  * \li Added SPI_GET_RX_FIFO_FULL_FLAG() and SPI_GET_RX_FIFO_FULL_FLAG() macros in spi.h.
  * \li Added SYS_MFP_P32_CPP1 definition in sys.h.
  * \li Added Hard_Fault_Sample.
  * \li Added RegBased samples including ACMP_TriggerTimerCompare, ADC_Compare, GPIO_Debounce, GPIO_Interrupt, GPIO_Toggle, GPIO_Wakeup, I2C_Master,
  *           I2C_Slave, PWM_DeadZone, SPI_MasterMode, SPI_SlaveMode, Timer_EventCounter, Timer_FreeCountingMode,
  *           Timer_ToggleOut, Timer_Wakeup, and WDT_Wakeup.
  * \li Added StdDriver samples including ACMP, ADC_Convert, GPIO_Debounce, GPIO_Interrupt, GPIO_Toggle, GPIO_Wakeup, I2C_Master, I2C_Slave,
  *           PWM_DoubleBuffer, SPI_MasterMode, SPI_SlaveMode, Timer_Periodic, Timer_TriggerCountingMode, Timer_Wakeup, and WDT_Polling.
  *
  * <b>Revision 3.00.002</b>
  * \li Renamed PWM_*FaultBreak*() functions to PWM_*FaultBrake*().
