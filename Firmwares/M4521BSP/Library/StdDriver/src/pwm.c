 ...
 * @brief    M4521 PWM driver source file
 *
 * @note
    ...
    for(i = 0; i < PWM_CHANNEL_NUM; i ++)
    {
        if(u32ChannelMask & (1 << i))
        {
            if((u32BrakeSource == PWM_FB_EDGE_SYS_CSS) || (u32BrakeSource == PWM_FB_EDGE_SYS_BOD) || \
                    (u32BrakeSource == PWM_FB_EDGE_SYS_COR) || (u32BrakeSource == PWM_FB_LEVEL_SYS_CSS) || \
                    (u32BrakeSource == PWM_FB_LEVEL_SYS_BOD) || (u32BrakeSource == PWM_FB_LEVEL_SYS_COR))
            {
