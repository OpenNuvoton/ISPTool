...
void UpdateConfig(unsigned int *data, unsigned int *res)
{
    ...
    u32Config[3] = FMC_CRC8(u32Config, 3);

    FMC_ENABLE_CFG_UPDATE();
    FMC_Erase_User(Config0);
    WriteData(Config0, Config0 + 16, u32Config);
...
int ParseCmd(unsigned char *buffer, uint8_t len)
{
    ...
    else if(lcmd == CMD_RESEND_PACKET)     //for APROM&Data flash only
    {
        ...
        if((StartAddress & 0xFF800) >= Config0)
            goto out;
        ReadData(StartAddress & 0xFF800, StartAddress, (uint32_t*)aprom_buf);
        FMC_Erase_User(StartAddress & 0xFF800);
        WriteData(StartAddress & 0xFF800, StartAddress, (uint32_t*)aprom_buf);
        if((StartAddress%FMC_FLASH_PAGE_SIZE) >= (FMC_FLASH_PAGE_SIZE-LastDataLen))
            FMC_Erase_User((StartAddress & 0xFF800)+FMC_FLASH_PAGE_SIZE);
        goto out;
