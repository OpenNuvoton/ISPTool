...
/**
  ...
  * @param[in]  u32Hclk is HCLK frequency. The range of u32Hclk is 25 MHz ~ 60 MHz.
  * @return     HCLK frequency
  * @details    This function is used to set HCLK frequency. The frequency unit is Hz.
  *             It would configure PLL frequency to 50MHz ~ 120MHz,
...
uint32_t CLK_SetCoreClock(uint32_t u32Hclk)
{
    ...
    /* The range of u32Hclk is 25 MHz ~ 60 MHz */
    if(u32Hclk > FREQ_60MHZ)
        u32Hclk = FREQ_60MHZ;
...
/**
  ...
  * |\ref PWM01_MODULE   |\ref CLK_CLKSEL1_PWM01_S_LXT          | x                      |
  ...
  * |\ref PWM23_MODULE   |\ref CLK_CLKSEL1_PWM01_S_LXT          | x                      |
...
uint32_t CLK_EnablePLL(uint32_t u32PllClkSrc, uint32_t u32PllFreq)
{
    ...
    if((u32PllFreq <= FREQ_500MHZ) && (u32PllFreq > FREQ_100MHZ))
    {
        u32NO = 0;
    }
