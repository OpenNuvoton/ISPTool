...
/**
  ...
  *                             - \ref SPIM_CTL1_IFSEL_GPIO
  *                             - \ref SPIM_CTL1_IFSEL_MCP
  *                             - \ref SPIM_CTL1_IFSEL_MCP64
  ...
  */
uint32_t SPIM_Open(SPIM_T *spim, uint32_t u32SPIMode, uint32_t u32BusClock, uint32_t u32IFSel)
{
    ...
    spim->CTL1 = (spim->CTL1 & ~SPIM_CTL1_IFSEL_Msk) | u32IFSel;    // Set Slave Peripheral to GPIO/MCP/MCP64.

#if 0   // User program will take care.
    if (u32IFSel == SPIM_CTL1_IFSEL_GPIO) {
        // Configure multi-function pins for SPIM.
...
/**
  ...
  * @param[in]  spim            Base address of SPIM module.
  * @param[in]  u32FlashAddr    Start Flash address to write.
  * @param[in]  u32Len          Number of bytes to write.
...
/**
  ...
  * @param[in]  u32FlashAddr    Start Flash address to read.
  * @param[in]  u32Len          Number of bytes to read.
  * @param[out] pu8RxBuf        Receive buffer.
  * @note                       Non-blocking function call. Call \ref SPIM_IS_BUSY to check finish or not.
  * @return     None.
  */
