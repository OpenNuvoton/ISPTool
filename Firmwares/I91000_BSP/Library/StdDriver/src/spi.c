 ...
 * @version  V1.00
 * $Revision: 1 $
 * $Date: 15/11/05 02:00p $
 * @brief    ISD9000 SPI driver source file
 *
 * @note
...
{
    return ((CLK_GetHCLKFreq()>>1)/((spi->CLKDIV&SPI_CLKDIV_DIVIDER0_Msk) + 1));
}
...
{
	return ((CLK_GetHCLKFreq()>>1)/(((spi->CLKDIV&SPI_CLKDIV_DIVIDER1_Msk)>>SPI_CLKDIV_DIVIDER1_Pos)+1));	
}
