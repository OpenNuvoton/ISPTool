...
uint32_t u32CardPwrPolarity[SC_INTERFACE_NUM] = {0, 0}; // cannot be static. used by SC_SET_VCC_PIN defined in sc.h
...
void SC_Open(SC_T *sc, uint32_t u32CD, uint32_t u32PWR)
{
    ...
    // Nano100AN does not have power inverse control bit
    if(u32PWR == SC_PIN_STATE_LOW) {
        u32Reg |= SC_PINCSR_POW_EN_Msk;
        u32CardPwrPolarity[u32Intf] = 0;
    } else {
        u32CardPwrPolarity[u32Intf] = 1;
    }
    sc->PINCSR = u32Reg;
...
/**
  ...
  * @param[in] sc Base address of smartcard module
  * @param[in] u32TimerNum Timer(s) to start. Valid values are 0, 1.
...
/**
  ...
  * @param[in] sc Base address of smartcard module
  * @param[in] u32TimerNum Timer(s) to stop. Valid values are 0, 1.
  * @return None
