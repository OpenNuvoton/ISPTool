 ...
 * $Revision: 1 $
 * $Date: 15/04/07 2:47p $ 
 * @brief    NM1200_NM1100 PWM driver source file
 *
 * @note
...
uint32_t PWM_GetADCTriggerFlag (PWM_T *pwm, uint32_t u32ChannelNum)
{
    ...
    if(u32ChannelNum < 4) {
            u32Ret = PWM->ADCTSTS0 >> (8 * u32ChannelNum);
    } else {
            u32Ret = PWM->ADCTSTS1 >> (8 * (u32ChannelNum - 4 ));
    }
...
{
    PWM->INTEN = (PWM->INTEN & ~PWM_INTEN_PINTTYPE_Msk) | (PWM_INTEN_ZIEN0_Msk << u32ChannelNum) | u32IntPeriodType;
}
