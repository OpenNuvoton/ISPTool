 ...
 * $Revision: 7 $
 * $Date: 15/07/15 7:20p $
 * @brief    MINI55 series PWM driver source file
 *
 * @note
 ...
 * @param[in] u32DutyCycle Target generator duty cycle percentage. Valid range are between 0 ~ 100. 10 means 10%, 20 means 20%, and so on.
 ...
 *                           Bit 0 is channel 0, bit 1 is channel 1, and so on.
 * @return None
 */
 ...
 *                         while fault brake occur. Bit 0 represents channel 0, bit 1 represents channel 1, and so on.
 ...
 * @param[in] pwm The base address of PWM module
 * @param[in] u32ChannelMask Combination of enabled channels. Each bit corresponds to a channel.
 *                           Set bit 0 to 1 enables channel 0 output, set bit 1 to 1 enables channel 1 output, and so on.
 * @return None
 */
 ...
 * @param[in] pwm The base address of PWM module
 * @param[in] u32ChannelMask Combination of enabled channels. Each bit corresponds to a channel
 *                           Set bit 0 to 1 disables channel 0 output, set bit 1 to 1 disables channel 1 output, and so on.
 * @return None
 */
...
{
    PWM->INTEN |= (PWM_INTEN_CMPDIEN0_Msk << u32ChannelNum);
}
...
{
    PWM->INTEN &= ~(PWM_INTEN_ZIEN0_Msk << u32ChannelNum);
}
