...
/**
  ...
  * \li Fix wrong baudrate returned by SCUART_SetLineConfig() in SCUART driver.
  * \li Fix SCUART_Open() of SCUART driver for wrong clock calculation and return value.
  * \li Fix SC_SET_STOP_BIT_LEN define error.
  * \li Fix bug of ADC_IS_DATA_OVERRUN(). The input parameter is channel number but channel bit mask.
  * \li Update CAN clock setting in CAN_Open() of CAN driver to compliant different system clocks.
  * \li Fix wrong return value bug of CLK_SetCoreClock() in CLK driver.
  * \li Fix CLK_SetModuleClock() error for PWM clock selection in CLK driver.
  * \li Fix COUNTFLAG may not be cleared bug of CLK_SysTickDelay() in CLK driver.
  * \li Fix GPIO_ENABLE_DOUT_MASK() and GPIO_DISABLE_DOUT_MASK() implement inverse error in GPIO driver.
  * \li Fix close wrong I2C bug of I2C_Close() in I2C driver.
  * \li Fix API declare name from I2C_SetClockBusFreq() to I2C_SetBusClockFreq() in I2C driver.
  * \li Fix clear RS-485 address byte detection flag bug to clear one flag in one time in RS485_HANDLE() of UART driver.
  * \li Add one more zero packet when BULK IN trasnfer is end by max packet size packet at last packet in VCOM sample code.
  * \li Fix UA_LIN_CTL[4] bit field name is 'MUTE_EN' not 'WAKE_EN' in UART LIN_CTL Bit Field Definitions of header file.
  * \li Fix wrong mask definition of SC_TRSR_TX_POINT_F_Msk and SC_TRSR_RX_POINT_F_Msk in header file.
  * \li Fix wrong bit definition of WAKEUP_EN in USB_INTEN register of header file.
  * \li Fix channel 0 trigger disabled bug when channel 1~3 trigger enabled in PWM_EnableADCTrigger() of PWM driver.
  * \li Fix a bug that channel 0 trigger will be disabled when channel 1~3 trigger are enabled in PWM_EnableADCTrigger() of PWM driver.
  * \li Fix SCUART_PARITY_NONE/SCUART_PARITY_EVEN/SCUART_PARITY_ODD definition bug in SCUART driver.
  * \li Fix four macro definitions of SPI driver to avoid affecting non-target SPI_SS pin including SPI_SET_SS0_HIGH() SPI_SET_SS1_HIGH() SPI_SET_SS0_LOW() SPI_SET_SS1_LOW().
  * \li Fix clear flag bug to clear one flag in one time in UART_ClearIntFlag(). It should be '(uart)->FSR = UART_FSR_RS485_ADD_DETF_Msk' but '(uart)->FSR |= UART_FSR_RS485_ADD_DETF_Msk'.
  * \li Fix clear wrong flag bug in UART_RS485_CLEAR_ADDR_FLAG().
  * \li Fix UA_LIN_CTL[4] bit field name is 'MUTE_EN' not 'WAKE_EN' in UA_LIN_CTL constants definitions in UART driver.
  * \li Add SPI_SET_SS_LEVEL() macro definition in SPI driver. This macro allows user to set both SPI_SS pins.
 ...
 */
    ...
    __IO uint32_t ISPSTA;        /* Offset: 0x40  Add ISPSTA control register of FMC                                 */
 ...
 * |        |          |For Master mode, I2S_BCLK and I2S_LRCLK pins are output mode and send bit clock from NuMicro NUC230_240 series to Audio CODEC chip.
 ...
 * |        |          |00 = Brown-out voltage is 4.4V.
 * |        |          |01 = Brown-out voltage is 3.7V.
 * |        |          |10 = Brown-out voltage is 2.7V.
 * |        |          |11 = Brown-out voltage is 2.2V.
 * |        |          |Note: This bit is the protected bit.
 * |        |          |It means programming this needs to write "59h", "16h", "88h" to address 0x5000_0100 to disable register protection.
 * |        |          |Refer to the register REGWRPROT at address GCR_BA+0x100.
