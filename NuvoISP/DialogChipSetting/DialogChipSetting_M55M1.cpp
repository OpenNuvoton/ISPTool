...
CDialogChipSetting_M55M1::CDialogChipSetting_M55M1(unsigned int uProgramMemorySize, unsigned int uFlashPageSize, unsigned int uPID, unsigned int uDID, unsigned int uChipSeries, CWnd *pParent /*=NULL*/)
    ...
    , m_uShowFlag(0x03)
{
    ...
    m_uConfigValue[11] = 0xFFFFFF5A;
    m_uConfigValue[12] = 0xFFFFFFFF;
    m_uConfigValue[13] = 0xFFFFFF5A;
    //}}AFX_DATA_INIT
}
...
BOOL CDialogChipSetting_M55M1::OnInitDialog()
{
    ...
    if (m_uShowFlag & 0x01) {
        m_TabChipSetting.InsertItem(nItem++, _T("Configuration 0-6, 11-13"));
        ...
        m_pChipSetting_CFG->m_uConfigValue[11]      = m_uConfigValue[11];
        m_pChipSetting_CFG->m_uConfigValue[12]      = m_uConfigValue[12];
        m_pChipSetting_CFG->m_uConfigValue[13]      = m_uConfigValue[13];
    ...
    if (m_uShowFlag & 0x02) {
        ...
        if ((m_uConfigValue[10] & 0xFFFF) == 0xFFFF) {
            m_pChipSetting_CFG_2 = new CDialogChipSetting_CFG_M55M1_2(TRUE);
        }
        else {
            m_pChipSetting_CFG_2 = new CDialogChipSetting_CFG_M55M1_2(FALSE);
        }
        ...
        m_pChipSetting_CFG_2->Create(CDialogChipSetting_CFG_M55M1_2::IDD, &m_TabChipSetting);
    }
...
void CDialogChipSetting_M55M1::OnOk()
{
    ...
    if (m_uShowFlag & 0x01) {
        ...
        m_uConfigValue[11] = m_pChipSetting_CFG->m_uConfigValue[11];
        m_uConfigValue[12] = m_pChipSetting_CFG->m_uConfigValue[12];
        m_uConfigValue[13] = m_pChipSetting_CFG->m_uConfigValue[13];
    }
