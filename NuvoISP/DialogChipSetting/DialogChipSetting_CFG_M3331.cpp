...
void CDialogChipSetting_CFG_M3331::ConfigToGUI()
{
	...
	m_bCheckISP_UART	= ((m_uConfigValue_t[3] & M3331_FLASH_CONFIG_UARTISPEN) != 0) ? TRUE : FALSE;
	m_bCheckISP_USB		= ((m_uConfigValue_t[3] & M3331_FLASH_CONFIG_USBISPDIS) == 0) ? TRUE : FALSE;
	m_bCheckISP_CAN		= ((m_uConfigValue_t[3] & M3331_FLASH_CONFIG_CANISPEN) != 0) ? TRUE : FALSE;
	m_bCheckISP_I2C		= ((m_uConfigValue_t[3] & M3331_FLASH_CONFIG_I2CISPEN) != 0) ? TRUE : FALSE;
	m_bCheckISP_SPI		= ((m_uConfigValue_t[3] & M3331_FLASH_CONFIG_SPIISPEN) != 0) ? TRUE : FALSE;
...
void CDialogChipSetting_CFG_M3331::GUIToConfig()
{
	...
	if (m_bCheckISP_UART)
		m_uConfigValue_t[3] |=  M3331_FLASH_CONFIG_UARTISPEN;
	else
		m_uConfigValue_t[3] &= ~M3331_FLASH_CONFIG_UARTISPEN;

	if (!m_bCheckISP_USB)
		m_uConfigValue_t[3] |=  M3331_FLASH_CONFIG_USBISPDIS;
	else
		m_uConfigValue_t[3] &= ~M3331_FLASH_CONFIG_USBISPDIS;

	if (m_bCheckISP_CAN)
		m_uConfigValue_t[3] |=  M3331_FLASH_CONFIG_CANISPEN;
	else
		m_uConfigValue_t[3] &= ~M3331_FLASH_CONFIG_CANISPEN;

	if (m_bCheckISP_I2C)
		m_uConfigValue_t[3] |=  M3331_FLASH_CONFIG_I2CISPEN;
	else
		m_uConfigValue_t[3] &= ~M3331_FLASH_CONFIG_I2CISPEN;

	if (m_bCheckISP_SPI)
		m_uConfigValue_t[3] |=  M3331_FLASH_CONFIG_SPIISPEN;
	else
		m_uConfigValue_t[3] &= ~M3331_FLASH_CONFIG_SPIISPEN;
	...
	if (m_bCheckSCEN)
	{
		...
		if (m_uConfigValue_t[4] >= (NUMICRO_FLASH_LDROM_ADDR + NUMICRO_SPECIAL_FLASH_OFFSET))
		{
			...
			if ((m_uConfigValue_t[4] + (m_uConfigValue_t[5] * m_uFlashPageSize)) > (NUMICRO_FLASH_LDROM_ADDR + NUMICRO_SPECIAL_FLASH_OFFSET + NUMICRO_FLASH_LDROM_SIZE_8K))
				m_uConfigValue[5] = (NUMICRO_FLASH_LDROM_ADDR + NUMICRO_SPECIAL_FLASH_OFFSET + NUMICRO_FLASH_LDROM_SIZE_8K - m_uConfigValue_t[4]) / m_uFlashPageSize;
		}
		else
		{
